  ...
  const Tool: React.FC<{ link: string; name: string }> = ({ link, name, children }) => (
    <Grid.Row as={Link} to={link} verticalAlign="middle" columns={2}>
      ...
      <Grid.Column textAlign="center">
        <Header>
          <Header.Content className="weak">{name}</Header.Content>
          <Header.Subheader className="slimjoe">Click here</Header.Subheader>
        </Header>
      </Grid.Column>
    </Grid.Row>
  );
  ...
  return (
    <>
      <Header as="h1" textAlign="center">
        <Header.Content className="strong">Toolset</Header.Content>
      </Header>
